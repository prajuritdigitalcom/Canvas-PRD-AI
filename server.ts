...
async function startServer() {
  ...
  // Helper to collect all keys from environment variables
  const getSystemApiKeys = (): string[] => {
    const keys: string[] = [];
    for (const envKey of Object.keys(process.env)) {
      if (envKey.toUpperCase().startsWith('GEMINI_API_KEY')) {
  ...
  app.post('/api/generate-prd', async (req, res) => {
    try {
      ...
      for (const attempt of attemptsQueue) {
        try {
          console.log(`Mencoba generasi menggunakan Kunci ${attempt.type === 'env' ? 'Server' : 'Pengunjung'} #${attempt.index}...`);
          const result = await runGeneration(attempt.key);
          responseText = result.text || '';
          successfulKeyType = attempt.type;
          successfulKeyIndex = attempt.index;
          console.log(`✅ Berhasil menggunakan Kunci ${attempt.type === 'env' ? 'Server' : 'Pengunjung'} #${attempt.index}!`);
          break; // Stop iterating upon first success
        } catch (err: any) {
          const errMsg = err?.message || String(err);
          console.error(`❌ Gagal menggunakan Kunci ${attempt.type === 'env' ? 'Server' : 'Pengunjung'} #${attempt.index}: ${errMsg}`);
      ...
      console.error('Global error pada api route:', globalErr);
      return res.status(500).json({
